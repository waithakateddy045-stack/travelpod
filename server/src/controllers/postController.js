...
const createPost = async (req, res, next) => {
    try {
        ...
        if (postType === 'BROADCAST' && !['ASSOCIATION', 'ADMIN'].includes(req.user.accountType)) {
            throw new AppError('Only associations can create broadcast posts', 403);
        }

        if (!videoUrl && postType !== 'BROADCAST') {
    ...
    // POST /api/reports — User reports any entity (post, comment, user)
    const reportEntity = async (req, res, next) => {
        try {
            const { entityId, entityType, reason, detail } = req.body;
            const userId = req.user?.id || null;

            if (!reason) throw new AppError('Reason is required', 400);
            if (!entityId || !entityType) throw new AppError('Entity ID and type are required', 400);

            // Map reason string to Enum
            const mappedReason = reason.toUpperCase().includes('SPAM') ? 'SPAM' :
                reason.toUpperCase().includes('INAPPRO') ? 'INAPPROPRIATE' :
                    reason.toUpperCase().includes('HARASS') ? 'HARASSMENT' :
                        reason.toUpperCase().includes('MISLEAD') ? 'MISLEADING' : 'SPAM';

            // Create actual Report record
            await prisma.report.create({
                data: {
                    ...
                    entityId,
                    entityType,
                    // Assign postId if it's a post or comment
                    postId: entityType === 'POST' ? entityId : null,
                    status: 'PENDING'
                }
            });

            // Store report as an analytics event
            await prisma.analyticsEvent.create({
                data: {
                    userId,
                    eventType: `${entityType}_REPORT`,
                    entityId,
                    entityType,
    ...
    module.exports = { createPost, getPost, deletePost, getModerationQueue, moderatePost, reportEntity };
